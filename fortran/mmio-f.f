...
c
      read (iunit,end=2000,fmt=200) tmp2
 200  format(1a)
c Change added Sun Oct 12 18:19:30 MDT 2008 by Mahmudul Hasan
c Previously the following line was j=2, which made it loop only through
c for the first 2 lines. 
c \todo j = 50 right now, but I think it should read whole file before
c \todo deciding about the start point of Matrix Information 
      j = 50
      do 10 i=1,j
        if ( tmp2(1:1) .ne. '%' ) then
           go to 20
