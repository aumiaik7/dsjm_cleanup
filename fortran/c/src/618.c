/* 618.f -- translated by f2c (version 20050501).
...
static integer c_b41 = 263743;
static integer c_b42 = 1302464;
...
/* Main program */ int MAIN__()
{
    ...
    integer f_open(olist *), s_wsle(cilist *), do_lio(integer *, integer *, 
	    char *, ftnlen), e_wsle(), s_cmp(char *, char *, ftnlen, ftnlen), 
	    f_clos(cllist *);
    ...
    static integer i__;
    static char rep[10];
    extern /* Subroutine */ int slo_(integer *, integer *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, integer *, integer 
	    *, integer *, integer *);
    static integer nnz, iwa1[263744], iwa2[263744], iwa3[263744], iwa4[263744]
	    , ndeg[263744];
    static complex cval[1302464];
    static integer ival[1302464], indx[1302464], jndx[1302464];
    static doublereal rval[1302464];
    static integer list[263744];
    static char symm[19], field[7];
    extern integer iargc_();
    static char ifile[32], ofile[32];
    static integer ncols, iunit, ipntr[263745], jpntr[263745], ounit__, nrows;
    extern /* Subroutine */ int mmread_(integer *, char *, char *, char *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, doublereal *, complex *, ftnlen, ftnlen, ftnlen);
    static integer indcol[1302465];
...
/* cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    ...
    e_wsle();
    slo_(&c_b41, indrow, jpntr, indcol, ipntr, ndeg, list, &maxclq, iwa1, 
	    iwa2, iwa3, iwa4);
    mmread_(&iunit, rep, field, symm, &nrows, &ncols, &nnz, &c_b42, indx, 
	    jndx, ival, rval, cval, (ftnlen)10, (ftnlen)7, (ftnlen)19);
    s_wsle(&io___29);
    do_lio(&c__9, &c__1, "  Matrix is type: ", (ftnlen)18);
    do_lio(&c__9, &c__1, rep, (ftnlen)10);
    do_lio(&c__9, &c__1, " ", (ftnlen)1);
	...
	if (s_cmp(field, "integer", (ftnlen)7, (ftnlen)7) == 0) {
	    s_wsle(&io___39);
	    do_lio(&c__3, &c__1, (char *)&indx[0], (ftnlen)sizeof(integer));
	    do_lio(&c__3, &c__1, (char *)&jndx[0], (ftnlen)sizeof(integer));
	    do_lio(&c__3, &c__1, (char *)&ival[0], (ftnlen)sizeof(integer));
	    e_wsle();
	    s_wsle(&io___40);
	    do_lio(&c__3, &c__1, (char *)&indx[1], (ftnlen)sizeof(integer));
	    do_lio(&c__3, &c__1, (char *)&jndx[1], (ftnlen)sizeof(integer));
	...
	e_wsle();
	mmwrite_(&ounit__, rep, field, symm, &nrows, &ncols, &nnz, indx, jndx,
		 ival, rval, cval, (ftnlen)10, (ftnlen)7, (ftnlen)19);
	cl__1.cerr = 0;
	cl__1.cunit = 9;
	cl__1.csta = 0;
	f_clos(&cl__1);
...
/*     ALGORITHM 618, COLLECTED ALGORITHMS FROM ACM. */
/*     ALGORITHM APPEARED IN ACM-TRANS. MATH. SOFTWARE, VOL.10, NO. 3, */
/*     SEP., 1984, P. 346-347. */
/*     PROGRAM TEST */
/*     ********** */
...
/*     END */
/* Subroutine */ int fcn_(integer *n, real *x, integer *indcol, integer *
	ipntr, real *fvec)
{
    /* System generated locals */
    integer i__1, i__2;
...
/* L10: */
	...
	sum += x[i__];
	fvec[i__] = sum * (sum + (float)1.) + (float)1.;
/* L20: */
    }
    return 0;
...
/* Subroutine */ int fdjs_(integer *m, integer *n, logical *col, integer *ind,
