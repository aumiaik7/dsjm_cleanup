...
c     
c     Variable Declaration from 618's Test Program. 
c
...
C
      ...
      CALL SLO(N,INDROW,JPNTR,INDCOL,IPNTR,NDEG,LIST,
...
C
      ...
      IF (IARGC() .GT. 1) THEN
         ...
         CALL MMWRITE(OUNIT,REP,FIELD,SYMM,NROWS,NCOLS,NNZ,
     *                 INDX,JNDX,IVAL,RVAL,CVAL)
         CLOSE(9)
...
C     FORMAT STATEMENTS.
C
C1000 FORMAT(// ' TESTS FOR DSM AND FDJS - NEUTRON KINETICS PROBLEM'//
...
C2000 FORMAT(// 3X,'N',6X,'NNZ',5X,'DNSM',5X,
C    *         'MINROW',4X,'MAXROW',4X,'MINGRP',4X,'MAXGRP'//)
C3000 FORMAT(2(I5,3X),F6.2,4X,4(I5,5X))
C4000 FORMAT(// ' *** MISTAKE IN INPUT DATA, INFO IS ***',I6)
C5000 FORMAT(// ' LARGEST RELATIVE ERROR OF APPROXIMATION IS',E10.2)
C     END
